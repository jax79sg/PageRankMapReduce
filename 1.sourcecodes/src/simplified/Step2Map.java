...
public class Step2Map extends Mapper<LongWritable, Text, LongWritable, Text>{
    ...
    public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
    	...
    	//Calculate mass to distribute, then emit fromNode	fromNode_[ToNodes]_PageRank (Pass along structure)
    	FloatWritable pMass = new FloatWritable(thisNode.getPagerank()/thisNode.getSizeOfAdjacencyList());
    	context.write(new LongWritable(thisNode.getNodeId()), thisNode.printToString()); //LongWritable, Node
    	CustomProperties.printDebug("Step2Mapy MainNode: \t" + thisNode.getNodeId() + "\t" + thisNode.printToString());
